const { AppError, ValidationError, NotFoundError, UnauthorizedError, ForbiddenError } = require('../utils/appError');
...
const sendErrorProd = (err, res) => {
  ...
  if (err.isOperational) {
    logger.info(err.message, { status: err.status, statusCode: err.statusCode });
    res.status(err.statusCode).json({
      status: err.status,
      message: err.message,
    });
